...
Serena works with any language that has an LSP server configured. Note that LSP servers often require:

- Installation in your IDE or project (not just globally)
- IDE extensions or plugins to enable the server
- Per-project configuration (e.g., `tsconfig.json`, `pyproject.toml`, `rust-toolchain.toml`)
- Environment settings (e.g., Python virtualenv path, Node.js version)
...
The specific features available depend on the language server's capabilities and configuration.
