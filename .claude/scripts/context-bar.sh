#!/bin/bash
# NOTE: This script intentionally does not use set -e because it must produce
# output even when individual commands fail (e.g., git not available, jq parsing errors).
# Failures in non-critical sections are handled gracefully with fallback values.
...
# Generate progress bar for a given percentage
# Usage: bar=$(generate_progress_bar <percentage>)
