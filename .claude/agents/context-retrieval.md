...
**Prefer project-scoped queries first**, then fall back to cross-project search:

- When working in a specific project, use `execute_forgetful_tool("query_memory", {project_id: <id>, ...})` first
- If scoped results are insufficient, then search without project_id to find cross-project patterns
...
Replace `[language]` below with the actual language (e.g., `python`, `javascript`, `sql`, `bash`):

```python
# Example: Use the correct language identifier for syntax highlighting
